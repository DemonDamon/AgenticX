...
overview: 对齐大众可接受的「侧栏入口开新会话 + 空态深度研究模式」交互：本期只做深度研究一个能力入口；租户总闸与 BFF 流水线不变。
todos:
  - id: lift-mode-sidebar
    content: WorkspaceShell：deepResearchMode + 侧栏深度研究=createSession 进模式
    status: pending
  - id: empty-dual-state
    content: MachiChatView：普通空态 pill / 深度研究空态芯片与文案同步 mode
    status: pending
...
flowchart TD
  ...
  NewChat["侧栏 新对话"] --> NormalEmpty["普通空态"]
  NormalEmpty --> Pill["空态下方 深度研究 pill"]
  Pill --> DREmpty
  DREmpty --> Chip["输入区 深度研究 芯片 + 专用占位文案"]
  Chip --> Send["发送带 deepResearch=true"]
```

用户选择：**本期空态能力条只露出「深度研究」一个 pill**，不做 PPT/文档/网站等占位。
...
- [`MachiChatView.tsx`](enterprise/apps/web-portal/src/components/MachiChatView.tsx)：普通空态 pill；深度研究空态（欢迎文案/占位/输入区芯片）；`deepResearch` 与父级 mode 同步后仍走现有 `sendMessage(..., { deepResearch })`。
...
- **普通空态**（`isEmpty && !deepResearchMode`）：在 `composer` **下方**增加单个 pill「深度研究」，点击 `onDeepResearchModeChange(true)`（**不再** `createSession`，当前已是空会话）。
- **深度研究空态**（`isEmpty && deepResearchMode`）：
  - 欢迎副文案改为深度研究说明（i18n）
  - **隐藏**建议卡片 `suggestions`
  - **隐藏**空态下方 pill
  - `InputArea` placeholder 用「描述你的问题，生成深度研究报告」类文案
  - 输入区 leftToolbar：显微镜旁增加可关闭的「深度研究」文字芯片（或把显微镜换成带文案的 secondary 芯片按钮）；关闭芯片 = 退出模式回到普通空态
...
- 新对话空态下方仅一个「深度研究」pill；点击后进入深度研究空态且**不**额外新建会话。
- AC-2：空会话消息数仍为 0；placeholder / 芯片出现；建议卡隐藏。
...
3. 点「新对话」→ 普通空态 + 下方单个 pill；点 pill → 图二式空态。
4. 深度研究空态发问 → 出现进度/报告（总闸开时）；关显微镜再问 → 普通回答。
