...
在 `hc-0818` 将项目群聊页现有的 6 个团队模板从“按关键词选择用户已有分身”的静态预填充，改为可真正创建团队的产品能力。点击模板后展示模板自带的角色；确认后为每个角色创建独立数字分身，再使用这些新分身创建智能路由群聊。
...
- 为产品需求、市场调研、团队知识库、项目交付、缺陷验收、全栈研发 6 个模板定义 3–4 个成员。
...
AC：6 个模板均有至少 3 个成员；模板 ID、成员 ID 在各自作用域内唯一；每个成员系统提示包含职责、交付物与协作边界。
