# Portal Trace 运行时过程：Litefuse 风格双栏 UI（路径 B）

Planned-with: Cursor Grok 4.5
Suggested-Impl-Model: Composer 2.5

> 续作：在已落地的 `TraceTimeline` 聚合 API 之上，仅升级 admin-console **展示交互**，不引入 Litefuse/Langfuse 进程或 `@langfuse/*` 依赖（对齐 ADR-0001）。

## Goal

Portal 日志详情抽屉与 `/traces/[traceId]` 全屏页，展示类似 Litefuse 的双栏过程视图：左侧 observation 树 + 耗时条，右侧选中节点详情（元数据 / tokens / attrs / sources）。
...
- OTLP 导出、自托管 Litefuse、iframe 嵌入
...
对齐 Langfuse 思路的子集：骨架树先出、I/O 另请求、截断预览；不做 ClickHouse/Web Worker/iframe。

## no-scope-creep

只改 admin-console UI/API/i18n；不改 db-schema、web-portal 写路径、网关；不引入 Litefuse。
