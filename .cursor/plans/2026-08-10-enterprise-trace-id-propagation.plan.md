...
2. 新建 PG 迁移 `drizzle/0041_gateway_audit_trace_id.sql`（当前最大为 `0040_enterprise_deep_research_runs.sql`）：
...
3. 新建 MySQL 迁移 `drizzle-mysql/0015_gateway_audit_trace_id.sql`（当前最大为 `0014_chat_messages_content_mediumtext.sql`），语义等价（MySQL 不支持 `ADD COLUMN IF NOT EXISTS`，按该目录既有迁移的写法处理）。
