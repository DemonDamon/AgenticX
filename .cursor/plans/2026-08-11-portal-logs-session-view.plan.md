...
- 新增 `enterprise/packages/db-schema/drizzle/0045_portal_request_logs_session_idx.sql`：
...
- 新增 `enterprise/packages/db-schema/drizzle-mysql/0019_portal_request_logs_session_idx.sql`：MySQL 无 `CREATE INDEX IF NOT EXISTS`，照抄 `drizzle-mysql/0018_portal_request_logs_mode.sql` 里已用的写法（存在性判断 / 幂等处理保持一致，不要自创新风格）。
- 两个 `meta/_journal.json` 追加条目：PG `idx: 44, tag: "0045_portal_request_logs_session_idx"`（当前 PG journal 有 44 条，最后一条 idx 为 43）；MySQL `idx: 19, tag: "0019_portal_request_logs_session_idx"`（当前 19 条，最后 idx 18）。**实施前用 `grep '"idx"' meta/_journal.json | tail -3` 核实真实末位，不要照抄本行数字。**

**AC-1**

- `enterprise/packages/db-schema/src/__tests__/migration-inventory.test.ts`：journal 断言 44 → 45，sqlFiles 断言 46 → 47（PG 侧；MySQL 侧断言同步 +1）。
- `enterprise/packages/db-schema/src/__tests__/schema-parity.test.ts`：MySQL `sqlFiles.sort()` 期望数组加入 `"0019_portal_request_logs_session_idx.sql"`，journal 断言加入 `expect.objectContaining({ idx: 19, tag: "0019_portal_request_logs_session_idx" })`；PG 侧同理加 `0045_...`。
