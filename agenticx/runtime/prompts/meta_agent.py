...
def _build_widget_capability_block() -> str:
    ...
    return (
        ...
        "- **凡是要向用户展示流程/链路/步骤/时序/架构/数据走向/代理路径，必须先调用 `show_widget`，再写正文解读。**\n"
        ...
        "- **推荐工作流**：先 `show_widget(title=..., widget_code=<svg...>)` 画出主架构/流程 → 正文分节解读各模块 → 如需第二张图（时序/对比/MitM 细节）再第二次 `show_widget`。\n"
        "- **SVG vs HTML**：\n"
...
def build_meta_agent_system_prompt(
    ...
    *,
    ...
    base_prompt = (
        ...
        "- 必须中文。\n"
        "- 先给结论，再给依据。\n"
        "- 技术方案/架构/流程类回答：**先** `show_widget` 出图，**后**文字；"
