...
def build_meta_agent_system_prompt(
    ...
    *,
    ...
    base_prompt = (
        ...
        "- **todo_write 实时同步规则（重要）**：每完成一项 todo 必须**立即**再调一次 `todo_write`，"
        "把该项 status 设为 `completed`，并把下一项设为 `in_progress`；**禁止**等到所有任务都做完后才一次性批量更新。\n"
        "  - 反例（用户体验差）：8 项 todo 全程只调用了一次 todo_write，最后任务卡仍显示 0/8 转圈圈。\n"
        "  - 正例：每完成一个 file_read / 一段分析 / 一次 file_write，紧跟着一次 todo_write 把 1/8 → 2/8 → ... 推进。\n"
        "- 简单任务：优先单子智能体，避免过度调度。\n"
