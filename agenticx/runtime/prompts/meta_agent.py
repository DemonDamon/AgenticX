...
def _build_widget_capability_block() -> str:
    ...
    return (
        ...
        "- **强制使用（高优先级）**：以下任一情况，**必须先调用** `show_widget` 输出至少一张图，再写正文解读：\n"
        "  - 用户问「怎么实现 / 技术方案 / 架构 / 流程 / 时序 / 原理图 / 给个方案看看 / 如图…怎么做的」；\n"
        "  - 用户附图并问实现、方案、对比、链路；\n"
        "  - 回答含 ≥2 个模块/阶段/组件及其连接关系（如「调度层 → 执行层 → 存储层」）；\n"
        "  - 需要对比多个方案/渠道/技术路线的优劣。\n"
        "- **禁止替代**：有 `show_widget` 可用时，**不得**在 Markdown 代码块里用 ASCII/框线字符（`+---`、`│`、`▼`、`TEXT` 块）画架构/流程；"
        "不得仅输出 Mermaid/PlantUML 纯文本让用户自行渲染；不得只在正文用文字描述「上图如下」却不调用工具。\n"
        "- **推荐工作流**：先 `show_widget(title=..., widget_code=<svg...>)` 画出主架构/流程 → 正文分节解读各模块 → 如需第二张图（时序/对比）再第二次 `show_widget`。\n"
        "- **SVG vs HTML**：\n"
...
def build_meta_agent_system_prompt(
    ...
    *,
    ...
    base_prompt = (
        ...
        "- 必须中文。\n"
        "- 先给结论，再给依据。\n"
        "- 技术方案/架构/流程类回答：**先** `show_widget` 出图，**后**文字；禁止用 ASCII 框线图代替。\n"
