...
from agenticx.cc_bridge.ndjson import line_looks_like_result_success
...
class MessageBody(BaseModel):
    text: str
    wait_seconds: float = Field(default=180.0, ge=1.0, le=3600.0)
...
def post_message(session_id: str, body: MessageBody) -> MessageResponse:
    ...
    return MessageResponse(ok=ok, tail=tail, result_text=_extract_result_text(tail) if ok else "")
