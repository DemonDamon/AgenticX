# 和创智派 Desktop — macOS Alpha Preview
...
| 机型 | 下载文件 |
|------|---------|
| Apple M1 / M2 / M3 / M4（ARM） | `和创智派-x.x.x-arm64.dmg` |
| Intel Mac（2020 年前机型） | `和创智派-x.x.x-x64.dmg` |
...
若未使用自包含 DMG，和创智派需要 `agx` 命令行工具来运行本地 AI 服务。打开终端，运行：
...
1. 双击 `.dmg` 将和创智派.app 拖入 Applications
2. 在 Finder 里找到和创智派.app，**右键 → 打开**
3. 在弹窗中点击"打开"确认（只需第一次）

**方式 B（终端命令）：**

```bash
xattr -cr /Applications/和创智派.app
```

### Step 4 — 启动和创智派

双击和创智派.app，等待约 5-15 秒完成初始化即可使用。
...
| 产物 | `.dmg` | `和创智派-<version>-win-x64.exe`（NSIS） |
...
**CI**：推送 `v*` tag 或手动 `workflow_dispatch` 选择 `windows-amd64` 时，会运行同一脚本并上传 `和创智派-*-win-x64.exe` 构件。Windows 代码签名未接入（与 mac 无证书构建一致）。
...
```bash
cd desktop
npm run build:mac:arm64   # M 系列芯片 → 和创智派-x.x.x-arm64.dmg
npm run build:mac:x64     # Intel 芯片  → 和创智派-x.x.x-x64.dmg
```
