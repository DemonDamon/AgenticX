...
export function CommandPalette({ open, query, commands, onQueryChange, onExecute, onClose }: Props) {
  ...
  return (
    <div className="absolute inset-x-0 bottom-full z-50 mb-2">
      <div className="mx-auto w-full max-w-2xl rounded-xl border border-border bg-surface-panel shadow-2xl">
            ...
            commands.map((cmd, idx) => (
              <button
                ...
                className={`flex w-full items-center justify-between px-3 py-2 text-left text-sm ${
                  idx === selectedIdx ? "bg-cyan-500/15 text-cyan-200" : "text-text-muted hover:bg-surface-card"
                }`}
              >
                <div className="flex min-w-0 items-center gap-2">
                  <span className="inline-flex h-5 w-5 shrink-0 items-center justify-center rounded bg-surface-hover text-[11px] text-text-muted">
                    {cmd.icon ?? "·"}
                  </span>
                ...
                <div className="ml-2 shrink-0 text-right">
                  <div className="text-[10px] uppercase tracking-wide text-text-faint">{cmd.category}</div>
                  <div className="text-[11px] text-text-faint">{cmd.shortcut ?? ""}</div>
                </div>
              </button>
            ))
          )}
