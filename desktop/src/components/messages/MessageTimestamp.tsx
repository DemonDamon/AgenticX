...
export function MessageTimestamp({ ts, align }: { ts?: number; align: "left" | "right" }) {
  ...
  return (
    <span className="pointer-events-none select-none whitespace-nowrap text-[12px] tabular-nums leading-none text-text-subtle opacity-0 transition-opacity duration-150 group-hover:opacity-100">
      {label}
    </span>
  );
}
