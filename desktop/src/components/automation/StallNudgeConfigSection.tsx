...
export function StallNudgeConfigSection({ value, onChange, disabled }: Props) {
  ...
  return (
    <div className="rounded-xl border border-border bg-surface-card px-4 py-3.5">
      ...
      <div className="mt-3 space-y-2 border-b border-border/60 pb-3">
        <div className="text-xs font-medium text-text-strong">停滞警告</div>
      ...
      <div className="mt-3 flex items-center justify-between gap-3">
        <div>
          <div className="text-xs font-medium text-text-strong">自动续跑</div>
          <p className="mt-1 text-xs leading-relaxed text-text-muted">
      ...
      {nudgeBelowDetect ? (
        <p className="mt-2 text-[11px] text-amber-300/90">
          自动续跑触发时间应不小于停滞警告阈值；保存时将自动抬到 {value.stall_detect_silence_seconds} 秒。
        </p>
      ) : null}
