...
const FIELD_CLASS =
  "w-full rounded-md border border-border bg-surface-panel px-2 py-1.5 text-sm text-text-subtle";
...
export function TurnArchiveSettingsPanel() {
  ...
  return (
    <Panel title="对话轮次记忆" collapsible defaultCollapsed={false}>
      ...
      <div className="space-y-3 text-sm text-text-subtle">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <div>启用对话轮次归档</div>
            <div className="mt-0.5 text-[11px] text-text-faint">默认关闭；开启后每轮结束异步写入 turns 索引</div>
          </div>
          <MiniSwitch
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            <div>召回条数上限</div>
            <div className="mt-0.5 text-[11px] text-text-faint">每轮注入系统提示的历史对话片段数</div>
          </div>
          <input
            type="number"
            min={1}
            ...
            onBlur={() => {
              const v = Math.max(1, Math.min(10, Math.round(form.recall_turns_limit) || DEFAULTS.recall_turns_limit));
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            <div>新近度半衰期（天）</div>
            <div className="mt-0.5 text-[11px] text-text-faint">越久远的轮次在排序中权重越低</div>
          </div>
          <input
            type="number"
            min={0.1}
            ...
            onBlur={() => {
              ...
              const v = Number.isFinite(raw) && raw > 0 ? Math.min(365, Math.max(0.1, raw)) : DEFAULTS.halflife_days;
        ...
        <button
          type="button"
          className="text-[11px] text-text-faint hover:text-text-subtle"
        ...
        {showAdvanced ? (
          <div className="space-y-3 rounded-lg border border-border bg-surface-card/60 px-3 py-2.5">
            <div className="flex items-center justify-between gap-4">
              <div>
                <div>最小分块字数</div>
                <div className="mt-0.5 text-[11px] text-text-faint">短于此长度的回复不单独归档</div>
              </div>
              <input
                type="number"
                min={1}
                ...
                onBlur={() => {
                  const v = Math.max(1, Math.min(500, Math.round(form.min_chunk_chars) || DEFAULTS.min_chunk_chars));
            ...
            <div className="flex items-center justify-between gap-4">
              <div>
                <div>每轮最大分块数</div>
                <div className="mt-0.5 text-[11px] text-text-faint">单轮对话写入 turns 表的 chunk 上限</div>
              </div>
              <input
                type="number"
                min={1}
                ...
                onChange={(e) => setForm((prev) => ({ ...prev, max_chunks_per_turn: Number(e.target.value) }))}
                onBlur={() => {
                  const v = Math.max(
                    1,
