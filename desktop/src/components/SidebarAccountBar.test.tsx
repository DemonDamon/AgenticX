...
import { BUNDLED_META_AVATAR_IM_ZOOM_CLASS, DEFAULT_META_AVATAR_URL } from "../constants/meta-avatar";
...
describe("sidebar account chrome", () => {
  ...
  it("keeps the identity pill as local profile when official account is signed out", () => {
    ...
    try {
      ...
      expect(html).toContain('data-avatar-fit="user"');
      expect(html).not.toContain(BUNDLED_META_AVATAR_IM_ZOOM_CLASS);
      expect(html).not.toContain(DEFAULT_META_AVATAR_URL);
      expect(html).not.toContain("agx-near-line-art");
      ...
      expect(collapsed).toContain('data-avatar-fit="user"');
      expect(collapsed).not.toContain(BUNDLED_META_AVATAR_IM_ZOOM_CLASS);
      expect(collapsed).toContain("账号菜单");
