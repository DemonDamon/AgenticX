...
export function TokenDashboardPanel({ open, onClose }: Props) {
  ...
  const breakdownLabel = useCallback(
    (item: UsageBreakdownItem) => {
      ...
      const entry = resolveProviderEntry(providers, item.key);
      return getProviderDisplayName(item.key, entry);
    },
    [providers],
  );
            ...
            <div className="rounded-xl border border-border bg-surface-card px-5 py-5">
                ...
                <div className="mt-5 flex h-3 w-full overflow-hidden rounded-full bg-surface-base">
                  {breakdown.map((b, i) => (
                    <div
                      ...
                      title={`${breakdownLabel(b)}: ${b.percent}%`}
                    />
                  ))}
                </div>
              ) : null}
              ...
              <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-5">
                {(breakdown.length ? breakdown : []).slice(0, 10).map((b, i) => (
                  ...
                  >
                    <div className="truncate font-semibold text-[var(--text-strong)]">{breakdownLabel(b)}</div>
