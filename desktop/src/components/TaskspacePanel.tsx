...
export function TaskspacePanel({
    ...
    return rows.map((item) => {
      ...
      return (
        <div key={item.path} className="flex items-center gap-1">
          <button
            className={`flex-1 rounded px-1 py-0.5 text-left text-xs hover:bg-slate-800 ${
              selectedFilePath === item.path ? "text-cyan-300" : "text-slate-400"
            }`}
            style={{ paddingLeft }}
            title={item.path}
          ...
          <button
            className="rounded px-1 py-0.5 text-[10px] text-slate-500 hover:bg-slate-800 hover:text-cyan-300"
  ...
  return (
    <div ref={panelRef} className="flex h-full min-h-0 w-full flex-col bg-slate-950/95">
      <div className="relative flex items-center gap-1 border-b border-border/60 px-2 py-2">
        ...
        {showAddForm ? (
          <div className="absolute right-2 top-10 z-10 w-[280px] rounded-md border border-border bg-slate-900 p-2 shadow-2xl">
            <div className="mb-1 text-[11px] text-slate-400">新增 Taskspace</div>
            <input
              value={newPath}
            ...
            <input
              ...
              placeholder="显示名称（可选）"
              className="mb-2 w-full rounded border border-border bg-slate-950 px-2 py-1 text-[11px] text-slate-200 outline-none focus:border-cyan-500/50"
            />
      ...
      <div className="flex-1 overflow-y-auto border-b border-border/60 px-2 py-2">
        {loading ? <div className="text-xs text-slate-500">加载中...</div> : null}
        {!loading && !activeTaskspace ? <div className="text-xs text-slate-500">暂无 Taskspace</div> : null}
      ...
      >
        ...
        <div className="pointer-events-none absolute left-1/2 top-1/2 h-2 w-10 -translate-x-1/2 -translate-y-1/2 rounded-full border border-cyan-400/70 bg-slate-900/95 shadow-[0_0_10px_rgba(34,211,238,0.25)]" />
      </div>
      <div className="flex shrink-0 flex-col px-2 py-2" style={{ height: previewHeight }}>
        <div className="mb-1 truncate text-xs text-slate-500">{selectedFilePath || "文件预览"}</div>
        <pre className="min-h-0 flex-1 overflow-auto rounded bg-slate-900 p-2 text-[11px] leading-5">
          <code
