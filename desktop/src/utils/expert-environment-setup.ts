...
export function buildExpertEnvironmentSetupDraft({
  ...
  return [
    ...
    "6. Node 等依赖按工作区 lockfile 选择 npm/pnpm/yarn，并优先做本地安装，不做无依据的全局安装。系统级工具（如 LibreOffice、ImageMagick）和 Python 包（如 magic-pdf/MinerU）检测到缺失时，直接发起安装，用户在桌面端确认后自动执行；不要只给命令行手动步骤让用户自己去终端敲。",
    "7. 安装边界：只允许安装「工作区依赖清单中声明的包」和「已启用能力明确需要的外部依赖」。如果用户要求的包不在工作区声明中（例如与项目无关的第三方工具），必须明确拒绝并说明原因——该包不在工作区依赖清单中，不在环境配置范围内。不要因为用户随口要求就装任何东西。",
    "8. 在创建环境、安装或升级任何依赖之前，先展示诊断结论、拟执行命令和影响范围；每一次安装操作都会触发桌面端确认弹窗，用户确认后才会执行。不要用一句「已安装」代替真实命令结果。",
    "9. 安装完成后运行最小验证（解释器路径、版本、关键 import/命令以及项目自带测试或健康检查），最后汇报成功项、失败项、实际环境路径和可复现命令。",
    "",
