...
![Figure 1: TeamBench overview — task suite, paired modes, artifact scoring, team-level metrics, and headline findings](<@./paper/figures/fig0_overview.png>)
...
$$\mathrm{CO} = \frac{\mathrm{tokens}(a_t \text{ pipeline})}{\mathrm{tokens}(a_s)}$$

**Protocol Compliance (PC)**: fraction of role-boundary and output-format constraints satisfied.
...
![Figure 2: CTR vs. coupling level for all five models with SEM error bars](<@./paper/figures/fig1_ctr_coupling.png>)
...
![Figure 3: The capability-transfer paradox — (a) overall CTR, (b) coordination tax, by model](<@./paper/figures/fig2_paradox.png>)
...
![Figure 4: Model × scenario CTR heatmap](<@./paper/figures/fig3_scenario_heatmap.png>)
...
**Table S1. Team vs. individual quality (Wilcoxon signed-rank on Q\_team − Q\_single; zero diffs excluded).**
...
5. **No human baseline**: human-agent comparison is future work.
