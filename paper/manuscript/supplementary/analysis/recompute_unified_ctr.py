#!/usr/bin/env python3
"""Unified-estimator recomputation (all CTRs reported in the paper).

Computes the unified statistics reported in the revision:
- geometric vs arithmetic CTR per arm, per model, with explicit zero handling
  (symmetric epsilon floor EPS=0.01; zero counts reported alongside, matching
  the "(k)" superscripts in Table 1)
- layer-wise CTR (L1-only, L2-only) vs refine-k
- L1 saturation rates per arm
- integrator-vs-single L2 paired comparison
- 7-arm variance decomposition (protocol x compute) plus design-aware
  between-arm spread of arm means (replacing the two-subset eta2 comparison)

Run:  python analysis/recompute_unified_ctr.py
Reads:  analysis/data/pilot_flash_v2/summary.jsonl, analysis/data/pilot_kimi_k3/summary.jsonl
"""
