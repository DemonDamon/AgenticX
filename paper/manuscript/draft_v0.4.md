...
> **Draft v0.4** — 2026-08-26
> 相比 v0.3:① 新增 §5.7 统计显著性检验(Wilcoxon / Mann-Whitney,全部手写实现);② Related Work 增加对比矩阵与完整参考文献(BibTeX);③ 新增 Figure 1 基准总览图;④ 按统计结果诚实校准发现表述(悖论降为"方向一致",合并检验升为主证据)。
...
> **中文导读(给作者的 review 要点)**
> 1. v0.4 新增四块:**§5.7 统计显著性**(Wilcoxon/Mann-Whitney 全手写,含结修正)、**Related Work 对比矩阵(Table R1)+ 24 条 BibTeX**、**Figure 1 总览框架图**、按统计结果**诚实校准的表述**。
> 2. 统计结论(重要):**团队次优合并检验 p=3.5e-06(非常硬)**、**PROJ 甜蜜点合并 p=0.006(硬)**;耦合梯度是趋势但 per-cell 不显著(如实写"underpowered");**能力转移悖论家族内不显著(如实降级为"方向一致跨三家族",不再声称 statistically confirmed)**——这种诚实写法反而更受审稿人尊重。
> 3. 图表全集:fig0 总览、fig1 CTR×耦合、fig2 悖论双面板、fig3 场景热力图(均 PDF+PNG)。
> 4. 待办(下一步):v1.0 = 全英润色 + LaTeX 化 + 附录填充;然后按你的要求把论文存到飞书文档。
