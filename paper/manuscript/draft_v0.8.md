...
> **Draft v0.8** — 2026-08-30
> 相对 v0.7：**Kimi K3 跨模型复刻完成并全文并入**——Table 1 升级为双模型对比（630 runs 总量）；新增 Finding 5（跨模型一致性：装配 η² 0.428 vs 计算 η² 0.002，臂排序完全复现）；新增 §5.1 t-DATA-L-01 单案例（机械装配全零分 vs integrator 满分）；Abstract/引言/贡献/结论/限制项全部校准为双模型叙事；图 2/图 3 更新为双模型版本。
> 数据：**pilot v2 双模型（DS v4 Flash + Kimi K3，各 15 任务 × 3 seeds × 7 臂 = 315 runs，合计 630 runs，scoring v2.2，judge 关闭）**
...
Multi-agent LLM frameworks orchestrate specialized role-playing agents for complex work, yet evaluations of these systems report contradictory conclusions. We identify an overlooked confounder: **no existing study controls or even reports how team outputs are assembled from their constituent roles.** We introduce **TeamBench**, a paired individual-vs-team benchmark comprising 15 parameterized office tasks across 5 workplace scenarios × 3 coupling levels, evaluated across 7 arms: a naïve single agent, two compute-matched single baselines (k-round self-revision and best-of-k), and four assembly protocols (last-role output, deterministic concatenation, a dedicated integrator role, and section-slot blackboard). We exhaustively run all 7 arms on **two model families** (DeepSeek-V4-Flash and Kimi-K3, 630 runs total) with Wilcoxon-signed-rank tests, bootstrap CIs, and BH-FDR correction. **Assembly protocol is the dominant factor** in Capability Transfer Rate (CTR, team quality over compute-matched single quality), and the effect replicates across both families: on identical role outputs, CTR_matched ranges from 0.69 (last-role) to 1.55 (integrator) on Flash and from 0.69 to 1.61 on K3, with the integrator arm significant on both (Flash: 1.55, 95% CI [1.37, 1.76], p = 2.1×10⁻⁷, d = +0.74; K3: 1.50, 95% CI [1.33, 1.67], p = 1.5×10⁻⁶, d = +0.65) <!-- src: paper/experiments/v2/pilot_{flash_v2,kimi_k3}/pairs.csv + stats_v2 -->. The assembly factor explains 40–43% of quality variance versus ≤0.3% for the compute strategy—a 130–200× gap replicated across families. Strikingly, neither k-round self-revision nor best-of-k achieves the gains of team collaboration under the correct assembly—specialization with semantic integration is qualitatively different from more sampling or more rounds, and the gap *widens* on the stronger self-revising model (K3 refine-k CTR 1.08 vs integrator 1.72). We also report a measurement-integrity episode: the blackboard protocol mechanically produced correct section headings without content, trivially gaming a structure-only L1 rubric in an earlier scorer version; we fix this via content-aware section scoring and publish the incident. All tasks, parameterized generators, runners, and scoring rubrics are released.
...
- An empirical study on **two model families** (DS v4 Flash and Kimi K3, 15 tasks × 3 seeds × 7 arms each, 630 runs total, L2 assertions active) showing that **assembly protocol dominates CTR variance on both** (Flash η² = 0.400, K3 η² = 0.428, versus ≤0.003 for compute strategy), integrator-assembled teams significantly outperform compute-matched single baselines on both (Flash CTR_matched 1.55, 95% CI [1.37, 1.76], p = 2.1×10⁻⁷, d = +0.74; K3 1.50, 95% CI [1.33, 1.67], p = 1.5×10⁻⁶, d = +0.65), while naive engineering defaults (last-role) produce artifactual "team-inferiority" effects on both (CTR_matched 0.69). <!-- src: paper/experiments/v2/pilot_{flash_v2,kimi_k3}/pairs.csv + stats_v2 -->
- A cross-model negative result on structured assembly: deterministic section-slot (blackboard) and raw concatenation perform near or below the single baseline on both families—on K3, mechanical protocols collapse to Q = 0 on one aggregation task (t-DATA-L-01) across all seeds while the integrator turns the *same role outputs* into a perfect artifact—because LLMs write role-specific rather than schema-matching headings; only the semantic integrator normalizes these free-form outputs.
...
![Figure 1: TeamBench overview. Stage 1 — task space: 5 office scenarios × 3 coupling levels yield 15 task families, expanded to 150 variants by parameterized generators. Stage 2 — paired execution over 7 arms: three single-mode baselines (1-shot, Refine-k, Best-of-k) and a team mode whose identical role outputs are assembled under 4 protocols (Last, Concat, Blackboard, Integrator). Stage 3 — artifact scoring: three layers (L1 structural content-aware, L2 factual assertions, L3 semantic judge) produce team-level metrics CTR, CO, TMS.](../figures/fig1_teambench_overview.jpg)
...
Figure 1 (Stage 2) illustrates all four protocols applied to the same role outputs.
...
> **Data note.** All numbers below are **pilot v2 final, two model families**: DS v4 Flash and Kimi K3, each 15 tasks (with migrated L2 assertions) × 3 seeds × 7 arms = 315 runs (630 total), scoring v2.2, no L3 judge. An earlier v1 pilot (same matrix, L2 inactive, Flash only) is used only for the scorer ablation in §5.3.
<!-- src: paper/experiments/v2/pilot_{flash_v2,kimi_k3}/summary.csv + pairs.csv -->

### 5.1 Main result: assembly protocol dominates CTR

**Table 1. Seven arms, pooled over 15 tasks × 3 seeds (n = 45 per arm per model).** No L3 judge. CTR_matched is computed against each model's own refine-k arm.
<!-- src: paper/experiments/v2/pilot_{flash_v2,kimi_k3}/summary.csv 7-arms + pairs.csv -->

| Arm | Flash Q | Flash L2 | Flash TR | Flash CTR_matched | **K3 Q** | **K3 L2** | **K3 TR** | **K3 CTR_matched** |
|---|---|---|---|---|---|---|---|---|
| single (1-shot) | 0.606 | 0.759 | 1.00× | — | 0.613 | 0.775 | 1.00× | — |
| single_refine_k | 0.582 | 0.720 | 3.33× | 1.00 (ref) | 0.637 | 0.795 | 4.27× | 1.00 (ref) |
| single_bon_k | 0.584 | 0.715 | 4.27× | ~1.00 | 0.624 | 0.806 | 5.18× | 0.97 |
| team_last | 0.417 | 0.566 | 1.46× | **0.69** | 0.420 | 0.580 | 2.53× | **0.69** |
| team_concat | 0.535 | 0.536 | 1.42× | 0.91 | 0.542 | 0.548 | 2.46× | 0.89 |
| team_blackboard | 0.478 | 0.602 | 1.44× | 0.81 | 0.449 | 0.594 | 2.49× | 0.75 |
| **team_integrator** | **0.881** | 0.741 | 2.14× | **1.55** | **0.905** | 0.785 | 4.44× | **1.61** |

CTR_matched shown as arithmetic means for cross-arm comparability (K3 mechanical arms contain genuine zero-score runs); geometric-mean CTR with log-domain bootstrap CI for the integrator arm: Flash 1.55 [1.37, 1.76], K3 1.50 [1.33, 1.67]. All four team arms deviate from the refine-k baseline significantly on both models (Wilcoxon signed-rank with BH-FDR at q = 0.05; K3: last p = 4.4×10⁻⁶, concat p = 5.0×10⁻³, blackboard p = 3.5×10⁻⁵, integrator p = 1.5×10⁻⁶).

**Finding 1. Assembly protocol is the dominant factor—and the gap is enormous.** On Flash, geometric-mean CTR_matched across 4 team protocols spans **0.69 → 1.55 (Δ 0.86)** over *identical role outputs*; on K3 the span is **0.69 → 1.61 (Δ 0.92)**. The integrator arm beats the compute-matched baseline on both models (Flash: p = 2.1×10⁻⁷, d = +0.74, CI [1.37, 1.76]; K3: p = 1.5×10⁻⁶, d = +0.65, CI [1.33, 1.67]); every mechanical protocol is significantly *worse* than even the 1-shot single on both (K3 Cliff's d: last −0.52, blackboard −0.42, concat −0.21). The default engineering choice of `last-role = team output` does not merely underperform—it produces a "team-inferiority" artifact (CTR 0.69 on *both* models) that flips sign under integrator assembly.

**Finding 2 (variance decomposition).** Over the same set of role outputs, the assembly protocol factor explains **40.0%** of artifact-quality variance on Flash (one-way ANOVA F = 39.1, p = 2.0×10⁻¹⁹, η² = 0.400) and **42.8%** on K3, whereas the single-agent compute strategy (1-shot / refine-k / bon-k) explains **0.3%** on Flash and **0.2%** on K3. Assembly explains 130–200× more variance than compute, on both families. See Figure 2(b).

**Finding 3. Team gains survive compute-matching.** On Flash, the integrator burns 2.14× tokens vs the 1-shot single and reaches CTR_naive 1.52, while refine-k (3.33×) and bon-k (4.27×) burn *more* compute yet score *below* the 1-shot single (CTR_naive 0.96). K3 sharpens this result: its stronger self-revision lifts refine-k to CTR_naive 1.08 (Flash: 0.96), yet the integrator still reaches 1.72—a +59% gap over the best compute-matched baseline on the *same* model. Better models narrow the self-revision deficit but do not close the semantic-integration gap; specialization plus integration is not reducible to more sampling or more revision rounds.

**Finding 4. Mechanical assembly fails at the semantics step, and L2 localizes where.** The L2 signature replicates across models: on both Flash and K3, concat's factual layer (L2 0.536 / 0.548) falls *below* last's (0.566 / 0.580)—the answer-block extractor takes the last ```json block, which in a naive concatenation is the final role's *partial* block, while earlier roles' numeric content is stranded. The integrator is the only protocol that reliably aggregates all roles' numbers into one consistent answer block. Mechanism case study (structure, Flash): on t-CONTENT-M-02, roles write headings like `## 主文案撰写员（main）— 交付物` rather than the schema-required `## 主文案+Slogan`; the blackboard's slot matcher routes these into `## 其他` and leaves required sections empty, L1 = 0.0.

**Finding 5 (cross-model replication).** The entire effect structure replicates across two model families: the arm ordering (integrator ≫ concat > blackboard > last) is identical on Flash and K3; all four team arms deviate significantly from refine-k on both models under BH-FDR; the assembly-vs-compute variance gap (130× vs 200×) holds on both; and per-coupling CTR_matched CIs exclude 1.0 in all six cells (2 models × 3 coupling levels). A single-case study makes the mechanism vivid (K3, t-DATA-L-01, seed 0): the same three role outputs score Q = 1.00 under the integrator, 0.42 under concat, and **0.00 under both last and blackboard** (the final role emits no answer block and the slot matcher fills no required section)—while every single-mode baseline lands at 0.38–0.79. The capability was present in the role outputs all along; the assembly protocol alone determined whether it survived to the artifact.

### 5.2 CTR after controlling assembly, by coupling

**Table 2. Quality by coupling level (5 tasks × 3 seeds per cell), both models.** <!-- src: pilot_{flash_v2,kimi_k3} coupling breakdown -->
...
| Coupling | single | refine-k | last | concat | **integrator** | blackboard | integrator CTR_matched |
|---|---|---|---|---|---|---|---|
| Low | 0.604 | 0.657 | 0.444 | 0.625 | **0.919** | 0.482 | **1.43** [1.21, 1.69] |
| Medium | 0.579 | 0.586 | 0.321 | 0.432 | **0.872** | 0.292 | **1.55** [1.29, 1.86] |
| High | 0.656 | 0.669 | 0.494 | 0.569 | **0.925** | 0.573 | **1.52** [1.20, 1.89] |

The integrator advantage is **uniform across coupling levels on both models** (all six cells p < 0.005, all six bootstrap CIs exclude 1.0); no Simpson paradox emerges when pooling. Notably, v0.5's "coupling law" (CTR increasing monotonically with coupling) does **not** reappear once assembly is controlled—on Flash the Low-coupling cell posts the highest CTR_matched (1.69), driven by the refine-k baseline's relative weakness on low-coupling tasks (0.522); on K3 the ordering is M ≥ H > L, reflecting K3's stronger low-coupling self-revision (0.657). The "coupling law" was an artifact of last-role assembly interacting with role ordering, not a property of teams.

![Figure 2: (a)(b) Quality Q by assembly protocol × coupling level (15 paired runs per cell), DS v4 Flash and Kimi K3 side by side. The integrator row dominates every coupling level on both models while all mechanical protocols fall below both single baselines. (c) Variance decomposition of Q: the assembly-protocol factor explains 40.0% of variance on Flash and 42.8% on K3 (η² = 0.400 / 0.428) versus 0.3% / 0.2% for the compute strategy—a 130–200× gap over identical role outputs, replicated across model families.](../figures/fig4_assembly_coupling.png)

![Figure 3: Integrator CTR_matched by coupling level, both models (geometric mean, log-domain paired bootstrap 95% CI, n = 15 per level per model). All six CIs exclude the break-even line at 1.0. Flash: Low 1.69 [1.28, 2.16], Medium 1.46 [1.30, 1.66], High 1.51 [1.21, 1.90]. K3: Low 1.43 [1.21, 1.69], Medium 1.55 [1.29, 1.86], High 1.52 [1.20, 1.89].](../figures/fig5_ctr_matched.png)
...
The field has been asking "do LLM teams outperform individuals," but most of the variance in the answer comes from *how the team's output is assembled*, not from the model's capability or even the task's coupling. Our controlled experiment over four assembly protocols, replicated on two model families (630 runs), shows that the integrator protocol achieves a mean CTR_matched of 1.55 (Flash) and 1.50 (K3, geometric mean) against a compute-matched self-revision baseline (p = 2.1×10⁻⁷ and 1.5×10⁻⁶, d = +0.74 and +0.65), while naive last-role assembly—the default behavior of many frameworks—yields CTR_matched = 0.69 on *both* models. The assembly factor explains 40–43% of quality variance versus ≤0.3% for the compute strategy, and the full arm ordering replicates across families. TeamBench provides the first reproducible testbed for disentangling these effects and will enable future work on topology, prompts, and cross-framework generalization without conflating them with a pipeline step that nobody had been measuring.

---

## Appendix A. Assets (must read before citing any number)

- Raw run-level JSONs: `paper/experiments/v2/pilot_flash/` (v1, scoring v2.2), `paper/experiments/v2/pilot_flash_v2/` (v2, migrated L2, Flash), and `paper/experiments/v2/pilot_kimi_k3/` (v2, migrated L2, K3).
...
- Figures: `paper/figures/make_figures_v2.py` (Figures 2–3; deterministic from `pilot_flash_v2/summary.jsonl` and `pilot_kimi_k3/summary.jsonl`).
...
- Case study artifacts (§5.1 Finding 5): `paper/experiments/v2/pilot_kimi_k3/t-DATA-L-01/`.
