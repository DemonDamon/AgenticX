...
> **Draft v0.7** — 2026-08-29
> 相对 v0.6：全文数字统一为 **pilot v2 最终结果**（摘要/引言/贡献/结论中残留的 v1 数字全部校准：CTR 0.69→1.55、CI [1.37,1.76]、p=2.1×10⁻⁷、d=+0.74）；新增 Figure 2（装配×耦合热力图 + 方差分解）与 Figure 3（integrator CTR_matched 分耦合档，log 域 bootstrap CI）；§7 限制项更新（pilot v2 已完成）。
> 数据：**pilot v2（DS v4 Flash × 15 任务（L2 断言生效）× 3 seeds × 7 臂 = 315 runs，scoring v2.2，judge 关闭）**
...
- An empirical study (DS v4 Flash, 15 tasks × 3 seeds × 7 arms = 315 runs, L2 assertions active) showing that **assembly protocol dominates CTR variance** (η² = 0.400 vs 0.003 for compute strategy), integrator-assembled teams significantly outperform compute-matched single baselines (CTR_matched 1.55, 95% CI [1.37, 1.76], p = 2.1×10⁻⁷, d = +0.74) while burning ~2.1× tokens, and naive engineering defaults (last-role) produce artifactual "team-inferiority" effects (CTR_matched 0.69). <!-- src: paper/experiments/v2/pilot_flash_v2/pairs.csv + stats_v2 -->
...
**Finding 2 (variance decomposition).** Over the same set of role outputs, the assembly protocol factor explains **40.0%** of artifact-quality variance (one-way ANOVA F = 39.1, p = 2.0×10⁻¹⁹, η² = 0.400), whereas the single-agent compute strategy (1-shot / refine-k / bon-k) explains **0.3%** (F = 0.2, p = 0.84, η² = 0.003). Assembly explains 130× more variance than compute. See Figure 2(b).
...
![Figure 2: (a) Quality Q by assembly protocol × coupling level (15 paired runs per cell). The integrator row dominates every coupling level while all mechanical protocols fall below both single baselines. (b) Variance decomposition of Q: the assembly-protocol factor explains 40.0% of variance (η² = 0.400) versus 0.3% for the compute strategy (η² = 0.003)—a 130× gap over identical role outputs.](../figures/fig4_assembly_coupling.png)

![Figure 3: Integrator CTR_matched by coupling level (geometric mean, log-domain paired bootstrap 95% CI, n = 15 per level). All three CIs exclude the break-even line at 1.0: Low 1.69 [1.28, 2.16], Medium 1.46 [1.30, 1.66], High 1.51 [1.21, 1.90].](../figures/fig5_ctr_matched.png)
...
4. **Model coverage in this draft.** One model family completed with migrated tasks (DS v4 Flash, pilot v2, 315 runs). A Kimi K3 replication run is queued to test cross-family generalization of the assembly effect.
...
The field has been asking "do LLM teams outperform individuals," but most of the variance in the answer comes from *how the team's output is assembled*, not from the model's capability or even the task's coupling. Our controlled experiment over four assembly protocols shows that the integrator protocol achieves a mean CTR_matched of 1.55 against a compute-matched self-revision baseline (p = 2.1×10⁻⁷, d = +0.74, 95% CI [1.37, 1.76]) while naive last-role assembly, the default behavior of many frameworks, yields CTR_matched = 0.69. TeamBench provides the first reproducible testbed for disentangling these effects and will enable future work on topology, prompts, and cross-framework generalization without conflating them with a pipeline step that nobody had been measuring.
...
- Figures: `paper/figures/make_figures_v2.py` (Figures 2–3; deterministic from `pilot_flash_v2/summary.jsonl`).
