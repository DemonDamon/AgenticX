...
FALLBACK_MODELS 里全是公网厂商。连续两次超时后不得无条件改写
session.provider_name / model_name：附件路由锁或托管 provider 必须挡住换家。
"""
...
def test_enterprise_session_is_never_moved_to_a_public_provider():
    ...
    session = _Session(ENTERPRISE_PROVIDER, "vendor/private-vl")
...
def test_enterprise_guard_survives_logout(_no_real_global_config):
    """托管入口退登会把 providers.enterprise 删掉；守卫仍须按 provider 名拦住。"""
...
def test_attachment_routing_lock_blocks_fallback():
    ...
    remember_lock(
        session,
        RoutingModelRef(
            id="private_deploy/qwen3",
            provider="private_deploy",
            model="qwen3",
            label="Qwen",
        ),
    )
