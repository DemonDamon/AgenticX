...
def test_resolver_uses_mimo_default_base_url(tmp_path: Path, monkeypatch):
    _setup_paths(tmp_path, monkeypatch)
    ConfigManager.set_value("default_provider", "mimo", scope="global")
    ConfigManager.set_value("providers.mimo.api_key", "sk-mimo", scope="global")
    ConfigManager.set_value("providers.mimo.model", "mimo-v2.6-pro", scope="global")

    provider = ProviderResolver.resolve()
    assert isinstance(provider, MimoProvider)
    assert provider.base_url == "https://api.xiaomimimo.com/v1"
    assert provider.model == "openai/mimo-v2.6-pro"


def test_mimo_provider_strips_mimo_prefix():
    provider = MimoProvider.from_config({"model": "mimo/mimo-v2.6-flash", "api_key": "k"})
    assert provider.model == "openai/mimo-v2.6-flash"
    assert provider.base_url == "https://api.xiaomimimo.com/v1"


def test_mimo_provider_keeps_custom_base_url():
    provider = MimoProvider.from_config(
        {
            "model": "mimo-v2.6-pro",
            "api_key": "tp-key",
