...
def _use_routing_policy(monkeypatch, raw):
    # 附件路由策略是后续波次才进主线的模块；本波次只接自动看图调用点。
    attachment_routing = pytest.importorskip(
        "agenticx.studio.attachment_routing",
        reason="attachment routing lands in a later wave",
    )
