...
function findLastSameRunAssistantIndex(messages: ChatMessage[], runId: string): number {
  for (let i = messages.length - 1; i >= 0; i -= 1) {
    ...
    if ((message.deep_research?.events?.length ?? 0) === 0) continue;
 ...
 *
 * 关键兜底：该轮消息只在流结束时才持久化——mid-run 刷新会让 user+assistant
 * 整条turn消失。当按 topic 找不到归属消息时，合成 user+assistant 壳挂载工作台，
 * 让对话重新出现（「继续查看」重连也能按 runId 找到这条 assistant）。
 */
export function mergeDeepResearchHydrate(
  ...
  const topicMatches = Boolean(topic) && lastUser?.content?.trim() === topic;
  ...
  const lastAssistantIsDrShell =
    targetIndex >= 0 &&
    Boolean(messages[targetIndex]!.deep_research) &&
    (isPlaceholderRunId(messages[targetIndex]!.deep_research?.runId) ||
      messages[targetIndex]!.deep_research?.runId === latest.runId);

  // 归属消息存在（assistant 已持久化、只是丢了工作台元数据）且 user 能对上 topic
  // 或末条就是本轮深研壳 → 挂到既有 assistant；否则合成完整一轮。
  if (targetIndex >= 0 && (topicMatches || lastAssistantIsDrShell)) {
    const target = messages[targetIndex]!;
    const existingRunId = target.deep_research?.runId;
    // live/早落库壳常用 runId=pending；不得当成「别的 run」而拒绝挂载。
    if (
      existingRunId &&
    ...
    const next = messages.slice();
    next[targetIndex] = { ...target, deep_research: workbench };
    return next;
  }

  // Don't overwrite a thread that already belongs to a different real run.
  if (targetIndex >= 0) {
    const existingRunId = messages[targetIndex]!.deep_research?.runId;
    if (
      existingRunId &&
      !isPlaceholderRunId(existingRunId) &&
      existingRunId !== latest.runId
