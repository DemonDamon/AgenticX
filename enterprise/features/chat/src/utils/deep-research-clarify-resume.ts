...
export function parseClarifyResumeResponse(
  ...
  const continuedMsg = alreadyContinuedMessage(gate);

  if (status >= 200 && status < 300) {
    if (dataObj?.alreadyContinued === true || dataObj?.resumed === false) {
      return { kind: "already_continued", message: continuedMsg };
    }
    ...
    if (planRaw && typeof planRaw === "object" && !Array.isArray(planRaw)) {
      ...
      const subQuestions = Array.isArray(p.subQuestions)
        ? p.subQuestions
      ...
      if (objective && subQuestions.length > 0) {
        ...
        return {
          kind: "resumed",
          version: version ?? (typeof p.version === "number" ? p.version : undefined),
          plan: {
            version:
              version ??
              (typeof p.version === "number" && Number.isFinite(p.version) ? p.version : 1),
            objective,
            scope: asStrings(p.scope),
            subQuestions,
            sourceStrategy: asStrings(p.sourceStrategy),
            deliverables: asStrings(p.deliverables),
            assumptions: asStrings(p.assumptions),
          },
        };
      }
    }
