...
| Feature | NPM | 实现状态 | 主要代码 |
|---|---|---|---|
| iam | `@agenticx/feature-iam` | ✅ 已实现 | `services/`, `api/`, `components/` |
| chat | `@agenticx/feature-chat` | ✅ 已实现 | `ChatWorkspace.tsx`, `store.ts`, `components/` |
| policy | `@agenticx/feature-policy` | ✅ 已实现 | `services/pg-store.ts`, `snapshot/writer.ts` |
| audit | `@agenticx/feature-audit` | ✅ 已实现 | `services/pg-store.ts`, `api/audit.ts` |
| metering | `@agenticx/feature-metering` | ✅ 已实现 | `services/metering.ts`, `api/metering.ts` |
| model-service | `@agenticx/feature-model-service` | ⚠️ Stub | 逻辑在 `admin-console/lib/model-providers-store.ts` |
| knowledge-base | `@agenticx/feature-knowledge-base` | ⚠️ Stub | 规划对接 Machi KB stage-1 |
| tools-mcp | `@agenticx/feature-tools-mcp` | ⚠️ Stub | 规划 MCP 市场 |
| agents | `@agenticx/feature-agents` | ⚠️ Stub | 规划分身/智能体 |
| settings | `@agenticx/feature-settings` | ⚠️ Stub | portal 有 `SettingsPanel.tsx` 本地实现 |
