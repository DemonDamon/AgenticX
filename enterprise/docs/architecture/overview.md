...
```
    ...
    └─────────┬─────────┘               │                  │
              ...
              ▼                         │                  │
    ┌───────────────────┐               │    ┌─────────────▼──────────┐
    │   apps/gateway    │◄──────────────┘    │   admin-console        │
    │   Go :8088        │  remote poll       │   Next.js :3001        │
    └─────────┬─────────┘  /api/internal/*   └────────────────────────┘
              │
              ▼
    OpenAI-compatible 上游
    (DeepSeek / Moonshot / Ollama / 自建代理 …)
```
...
| Feature | 职责 | 状态 |
|---|---|---|
| iam | 用户/部门/角色/批量导入 | 已实现 |
| chat | 聊天工作区 UI + store | 已实现 |
| policy | 策略 PG 存储 + 快照发布 | 已实现 |
| audit | 网关审计查询 | 已实现 |
| metering | Token 四维查询 | 已实现 |
| model-service | Provider 管理 | Stub（逻辑在 admin-console） |
| knowledge-base | 知识库 | Stub |
| tools-mcp | 工具/MCP | Stub |
| agents | 智能体 | Stub |
| settings | 设置面板 | Stub（portal 有本地组件） |
...
默认种子租户/部门由 `db:seed` 写入；开发登录见根 [README.md](../../README.md)。
