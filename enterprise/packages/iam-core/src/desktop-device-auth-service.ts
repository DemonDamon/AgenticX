/**
 * Desktop browser device-authorization state machine.
 *
...
async function defaultStore(): Promise<DeviceAuthStore> {
  const { resolveDatabaseConfig } = await import("./database/config");
  if (resolveDatabaseConfig().dialect === "mysql") {
    ...
    } = await import("./repos/mysql/desktop-device-auth");
    return {
  ...
  const { desktopDeviceAuth: deviceTable } = await import("@agenticx/db-schema");
  const { and, eq } = await import("drizzle-orm");
  const { getIamDb } = await import("./db");
  return {
    async insert(values) {
      const db = getIamDb();
...
export async function initDesktopDeviceAuth(
  ...
  const deviceName = (input.deviceName ?? "Desktop").trim() || "Desktop";
