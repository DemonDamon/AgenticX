  ...
  insert: () => ({
    values: (row: StoredRow) => ({
      onDuplicateKeyUpdate: async () => {
        ...
        if (!mocks.rows.has(row.tenantId)) mocks.rows.set(row.tenantId, row);
      },
    }),
  }),
  update: () => ({
...
vi.mock("@agenticx/db-schema/mysql", () => ({
...
vi.mock("./database", () => ({
  getAdminMysqlDb: () => fakeDb,
}));

describe("MySQL token quota tenant isolation", () => {
  beforeEach(() => {
    vi.resetModules();
    mocks.rows.clear();
    mocks.insertWinner = null;
