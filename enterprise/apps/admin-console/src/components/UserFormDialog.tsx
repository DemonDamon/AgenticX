...
export function UserFormFields({
  ...
  const update = (patch: Partial<UserFormValues>) => onChange({ ...values, ...patch });

  return (
    <div className="space-y-3">
      <div className="space-y-1.5">
        <Label htmlFor={`${idPrefix}-email`}>{t("form.emailLabel")}</Label>
        <Input
          id={`${idPrefix}-email`}
          type="email"
          required
      ...
      <div className="space-y-1.5">
        <Label htmlFor={`${idPrefix}-name`}>{t("form.nameLabel")}</Label>
        <Input
          id={`${idPrefix}-name`}
          required
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div className="space-y-1.5">
          ...
          <Select value={values.status} onValueChange={(value) => update({ status: value as UserFormStatus })}>
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div className="space-y-1.5">
          <Label htmlFor={`${idPrefix}-phone`}>{t("detail.phone")}</Label>
          <Input
            id={`${idPrefix}-phone`}
            value={values.phone}
        ...
        <div className="space-y-1.5">
          <Label htmlFor={`${idPrefix}-employee-no`}>{t("detail.employeeNo")}</Label>
          <Input
      ...
      <div className="space-y-1.5">
        <Label htmlFor={`${idPrefix}-job-title`}>{t("detail.jobTitle")}</Label>
        <Input
          ...
          {roleOptions.map((role) => {
            ...
            return (
              <button
                ...
                onClick={() => {
                  const next = new Set(values.roleCodes);
                  if (next.has(role.code)) next.delete(role.code);
                  else next.add(role.code);
                  const roleCodes = [...next];
                  update({ roleCodes: roleCodes.length ? roleCodes : ["member"] });
                }}
              >
                <span
                  className={[
