...
import { ChevronDown, ChevronRight, FolderTree, Pencil, Plus, RefreshCw, Save, Trash2 } from "lucide-react";
...
function TreeBranch({
  ...
  return (
    <div>
      ...
      {expanded ? children.map((child) => <TreeBranch key={child.id} node={child} depth={depth + 1} selectedId={selectedId} expandedIds={expandedIds} onSelect={onSelect} onToggle={onToggle} />) : null}
    </div>
  );
}
...
export function OrganizationEditor() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const response = await adminFetch("/api/admin/departments?shape=tree", { cache: "no-store" });
    ...
    <Card>
      <CardHeader className="flex-row items-start justify-between gap-4 space-y-0">
        <div>
          ...
          <CardDescription className="mt-1">在这里新建、改名、移动和删除组织；批量开通会继续在本页下方完成。</CardDescription>
        </div>
      ...
      <CardContent className="grid min-h-[390px] gap-4 border-t border-border pt-5 lg:grid-cols-[minmax(0,0.9fr)_minmax(320px,1.1fr)]">
        <div className="rounded-xl border border-border p-2">
          {loading ? <div className="space-y-3 p-3"><Skeleton className="h-8 w-full" /><Skeleton className="h-8 w-4/5" /><Skeleton className="h-8 w-2/3" /></div> : tree.length ? tree.map((node) => <TreeBranch key={node.id} node={node} depth={0} selectedId={selectedId} expandedIds={expandedIds} onSelect={setSelectedId} onToggle={toggleExpanded} />) : <div className="p-6 text-center text-sm text-muted-foreground">还没有组织，请先创建顶层组织。</div>}
        </div>
