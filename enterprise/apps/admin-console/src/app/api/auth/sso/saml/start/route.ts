...
export async function GET(request: Request) {
  ...
  if (isSamlGloballyDisabled()) {
    return NextResponse.redirect(
      new URL(`/login?sso_error=${encodeURIComponent("saml.provider_not_configured")}`, url.origin)
    );
  }

  try {
    const provider = await getAdminSamlProviderConfigServer(providerId);
    const secret = resolveStateSecret();
    const handler = createSamlProtocolHandler();
    const result = await handler.startAuthentication({
      provider,
      cookieSecret: secret,
      cookieName: ADMIN_SAML_STATE_COOKIE,
      returnTo: "/dashboard",
    });
