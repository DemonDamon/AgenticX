...
vi.mock("../../../../../../lib/admin-sso-runtime", () => ({
  getAdminSamlProviderConfigServer: vi.fn(async () => {
    throw new Error("must not be called");
  }),
}));

vi.mock("../../../../../../lib/admin-pg-auth", () => ({
  authenticateAdminConsoleViaOidc: vi.fn(),
}));
...
describe("admin SAML routes honour SSO_SAML_DISABLED", () => {
  it("start route redirects to /login?sso_error=saml.provider_not_configured", async () => {
    ...
    expect(response.headers.get("location")).toContain("sso_error=saml.provider_not_configured");
  });
  ...
  it("callback route returns 400 saml.provider_not_configured and writes failure audit", async () => {
    ...
    const response = await POST(
      new Request("https://admin.example.com/api/auth/sso/saml/callback", {
        method: "POST",
        body: formData,
      })
    );
