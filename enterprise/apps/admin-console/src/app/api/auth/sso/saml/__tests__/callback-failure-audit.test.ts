...
vi.mock("../../../../../../lib/admin-sso-runtime", () => ({
  getAdminSamlProviderConfigServer: vi.fn(),
}));
...
describe("admin saml callback parse failure", () => {
  it("audits auth.sso.login_failed and redirects with sso_error", async () => {
    ...
    expect(response.headers.get("location")).toContain("/login?sso_error=saml.callback_failed");
