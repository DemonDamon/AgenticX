...
func TestEntitlementSQLDoesNotQueryLegacyFeatureAssignments(t *testing.T) {
	raw, err := os.ReadFile("entitlement.go")
	if err != nil {
		t.Fatalf("read entitlement.go: %v", err)
	}
	if strings.Contains(string(raw), "enterprise_feature_assignments") {
		t.Fatal("entitlement must not query enterprise_feature_assignments")
	}
}
