...
func msg(role, content string) openai.ChatMessage {
	return openai.ChatMessage{Role: role, Content: openai.NewStringContent(content)}
}
...
func TestLatestUserMessageContent_OnlyReturnsLastUserContent(t *testing.T) {
	messages := []openai.ChatMessage{
		msg("system", "你是助手"),
		msg("user", "我的银行卡号是 6222021234567890123"),
		msg("assistant", "请求触发合规拦截"),
		msg("user", "那今天天气如何"),
	}
...
func TestReplaceLastUserMessageContent_PreservesMultiTurn(t *testing.T) {
	messages := []openai.ChatMessage{
		msg("system", "你是助手"),
		msg("user", "我的卡号是 6222021234567890123"),
		msg("assistant", "好的"),
		msg("user", "再帮我看下卡号 6222021234567890123 的归属地"),
	}
	...
	if openai.ContentText(next[1].Content) != openai.ContentText(messages[1].Content) {
		t.Fatalf("expected earlier user message untouched, got=%q want=%q", openai.ContentText(next[1].Content), openai.ContentText(messages[1].Content))
	}
	if next[3].Role != "user" {
		t.Fatalf("expected last role to remain user, got=%q", next[3].Role)
	}
	if openai.ContentText(next[3].Content) != "再帮我看下卡号 [REDACTED] 的归属地" {
		t.Fatalf("expected last user content replaced, got=%q", openai.ContentText(next[3].Content))
	}
}

func TestReplaceLastUserMessageContent_NoUserIsNoop(t *testing.T) {
	messages := []openai.ChatMessage{
		msg("system", "system only"),
	}

	next := replaceLastUserMessageContent(messages, "ignored")
	if len(next) != 1 || openai.ContentText(next[0].Content) != "system only" {
