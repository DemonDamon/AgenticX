...
vi.mock("@agenticx/iam-core", () => ({
  ...
  setUserOptOut: vi.fn(async () => undefined),
}));
...
describe("/api/desktop/capabilities", () => {
  ...
  it("lists capabilities the user turned off, so they can be turned back on", async () => {
    loadUserCapabilityView.mockResolvedValue({
      assigned: [
        { id: MCP_ID, kind: "mcp", name: "m", displayName: "M", requires: [], surfaces: ["web", "desktop"] },
        { id: SKILL_ID, kind: "skill", name: "s", displayName: "S", requires: [], surfaces: ["web", "desktop"] },
      ],
