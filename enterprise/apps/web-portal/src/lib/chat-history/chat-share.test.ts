...
describe.each(["postgresql", "mysql"] as const)("chat sharing (%s)", (dialect) => {
  it("creates an immutable snapshot from exactly the requested messages and reads it back", async () => {
    ...
    expect(loaded?.messages.map((message) => message.content)).toEqual(["world"]);
