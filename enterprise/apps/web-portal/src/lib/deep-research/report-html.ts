  ...
  while (i < lines.length) {
    ...
    if (line.startsWith("```")) {
      ...
      if (langLower === "mermaid") {
        ...
        // 渲染链：xychart-beta（走 Mermaid）→ 内置轻量 SVG → GFM 数据表 → 原样代码块。
        const spec = parseChartSpec(source);
        const xy = spec ? chartSpecToXyChart(spec) : null;
        if (xy) {
          const escaped = escapeHtml(xy);
          blocks.push(
            `<div class="mermaid-wrap"><pre class="mermaid">${escaped}</pre><pre class="mermaid-fallback" hidden>${escaped}</pre></div>`,
          );
        } else if (spec) {
          ...
          if (svg) {
            ...
            const rendered = markdownToHtml(table).html;
    ...
    // Lone `|…|` lines that are not a GFM table (no separator row) break the
    // paragraph scanner immediately without advancing — consume one line so we
    // never spin forever (e.g. section drafts with broken comparison tables).
    if (i === paraStart) {
      para.push(line);
      i += 1;
    }
...
export function renderHtmlReport(input: HtmlReportInput): string {
  ...
  const needMermaid = Boolean(input.mindmapMermaid.trim()) || bodyHasMermaid || bodyHasChart;
