 ...
 * - auto：开放式题默认给 card 一次（非阻塞，可跳过/超时）。
 */
export function assessClarifyStrategy(input: {
  ...
  if (preference === "auto" && !openEnded && !highRisk && missingSlots.length === 0) {
  ...
  // 6) auto 下开放题或缺槽：默认 card，非阻塞。
  if (openEnded || missingSlots.length > 0) {
    return {
      mode: "card",
      phase: "preflight",
      blocking: false,
