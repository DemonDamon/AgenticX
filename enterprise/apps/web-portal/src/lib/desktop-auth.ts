...
/** Scopes issued for Desktop device PATs (control + managed inference). */
...
export function desktopPatDisplayName(deviceName: string): string {
  const name = deviceName.trim() || "Desktop";
  return `Desktop · ${name}`;
}
