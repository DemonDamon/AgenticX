...
export GATEWAY_INTERNAL_BASE_URL="${GATEWAY_INTERNAL_BASE_URL:-http://192.168.16.66:8088}"
export GATEWAY_INTERNAL_URL="${GATEWAY_INTERNAL_URL:-http://192.168.16.66:8088}"
export GATEWAY_REMOTE_PROVIDERS_URL="${GATEWAY_REMOTE_PROVIDERS_URL:-http://192.168.16.70:3001/api/internal/providers}"
export GATEWAY_REMOTE_CHANNELS_URL="${GATEWAY_REMOTE_CHANNELS_URL:-http://192.168.16.70:3001/api/internal/channels}"
export GATEWAY_REMOTE_PRICING_CONFIG_URL="${GATEWAY_REMOTE_PRICING_CONFIG_URL:-http://192.168.16.70:3001/api/internal/pricing-snapshot}"
export GATEWAY_REMOTE_POLICY_SNAPSHOT_URL="${GATEWAY_REMOTE_POLICY_SNAPSHOT_URL:-http://192.168.16.70:3001/api/internal/policy-snapshot}"
export GATEWAY_REMOTE_MCP_SERVERS_URL="${GATEWAY_REMOTE_MCP_SERVERS_URL:-http://192.168.16.70:3001/api/internal/mcp-servers-snapshot}"
export GATEWAY_CHANNEL_REGISTRY="${GATEWAY_CHANNEL_REGISTRY:-on}"
export GATEWAY_COMPLETIONS_URL="${GATEWAY_COMPLETIONS_URL:-http://192.168.16.66:8088/v1/chat/completions}"
#export NEXT_PUBLIC_ADMIN_CONSOLE_URL="${NEXT_PUBLIC_ADMIN_CONSOLE_URL:-https://test.pal.cmccfund.com:3001}"
export NEXT_PUBLIC_ADMIN_CONSOLE_URL="${NEXT_PUBLIC_ADMIN_CONSOLE_URL:-http://192.168.16.70:3001}"
export WEB_PORTAL_PUBLIC_BASE_URL="${WEB_PORTAL_PUBLIC_BASE_URL:-http://192.168.16.68:3000}"
