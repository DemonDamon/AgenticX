...
> 本轮重大变更：新增 **MinerU 解析结果存储子模块 `mineru/`**（数据模型 / 工件注册器 / Markdown 渲染器 / 结构化输出验证器 / 调试可视化器）；`redis`/`psycopg2`/`pymilvus` 改为惰性可选导入；Milvus 存储补齐高性能检索、`flush`/`load` 可选参数与查询属性修复；Neo4j 增强 GraphRAG 批处理与知识图谱能力；StorageManager 改进初始化与调试。
...
├── mineru/                       # (NEW) MinerU 解析结果存储子模块
│   ├── __init__.py               # 子模块入口
│   ├── models.py                 # MinerU 解析结果数据模型
│   ├── registry.py               # 工件注册器与存储管理
│   ├── renderer.py               # Markdown 渲染器
│   ├── validator.py              # 结构化输出验证器
│   └── visualizer.py             # 调试可视化器
...
#### `vectordb_storages/milvus.py` (3015 bytes，(变更) 多轮增强)
**文件功能**：Milvus向量存储实现
**技术实现**：基于Milvus的高性能向量搜索引擎；本轮补齐高性能检索实现、修复查询属性访问、支持 GraphRAG 批处理；**(变更)** `flush`/`load` 操作改为可选参数以降低无谓开销
**关键组件**：MilvusStorage类，支持大规模向量检索
**业务逻辑**：提供企业级向量搜索服务，适用于大规模AI应用
**依赖关系**：继承BaseVectorStorage；**(变更)** `pymilvus` 改为惰性可选导入
...
### MinerU 解析结果存储子模块（NEW）

承接 MinerU 文档解析 pipeline 的结构化产物落地，将解析结果统一建模、注册、渲染与校验，便于后续入库与调试。

#### `mineru/__init__.py`（NEW）
**文件功能**：MinerU 存储子模块入口，导出数据模型、注册器、渲染器、验证器与可视化器

#### `mineru/models.py`（NEW）
**文件功能**：定义 MinerU 解析结果的数据模型
**技术实现**：基于 Pydantic 建模解析产物（页面、块、表格、图片、版面元素等）及其元数据，提供类型安全的结构化表示
**业务逻辑**：为解析结果在存储/渲染/校验各环节提供统一的数据契约

#### `mineru/registry.py`（NEW）
**文件功能**：工件（Artifact）注册器与存储管理
**技术实现**：登记并管理 MinerU 解析产生的工件（如 Markdown、图片、结构化 JSON），统一其存储位置与生命周期
**业务逻辑**：把零散的解析产物收敛为可追溯、可检索的工件集合

#### `mineru/renderer.py`（NEW）
**文件功能**：Markdown 渲染器
**技术实现**：将 MinerU 结构化解析结果渲染为 Markdown 文本，保留版面与表格等结构信息
**业务逻辑**：为知识库入库、预览与下游 LLM 消费提供可读的 Markdown 输出

#### `mineru/validator.py`（NEW）
**文件功能**：结构化输出验证器
**技术实现**：对解析/渲染结果进行结构化校验，识别缺失字段、格式异常等问题
**业务逻辑**：在产物落地前做质量门禁，提升解析结果可靠性

#### `mineru/visualizer.py`（NEW）
**文件功能**：调试可视化器
**技术实现**：将解析结果可视化呈现（版面框、元素分布等），辅助排查解析质量问题
**业务逻辑**：为 MinerU 解析 pipeline 的调试与回归提供直观的可视化手段

---

## 模块架构特点

### 0. 本轮（2025-09-16 之后）关键演进
- **(NEW) MinerU 存储子模块**：新增 `mineru/`，覆盖数据模型 → 工件注册 → Markdown 渲染 → 结构化校验 → 调试可视化的完整链路
- **(变更) 惰性可选依赖**：`redis`、`psycopg2`、`pymilvus` 改为按需惰性导入，未安装时不阻塞模块加载（对齐打包/轻量部署场景）
- **(变更) Milvus 增强**：补齐高性能向量检索实现、`flush`/`load` 操作改为可选参数（降低无谓开销）、查询属性访问修复，并支持 GraphRAG 批处理
- **(变更) Neo4j 增强**：增强 GraphRAG 系统的批处理与知识图谱存储能力
- **(变更) StorageManager**：改进初始化与调试逻辑
