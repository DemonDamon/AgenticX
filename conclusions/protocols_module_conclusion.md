...
> 结论更新时间：2026-05-29（覆盖 2025-12-29 之后的变更）（无重大变更）
...
├── agui.py (NEW) ← ag-ui协议内化：Agent-User交互标准化
├── client.py (11360 bytes)
├── interfaces.py (2870 bytes)
├── mining_protocol.py (NEW) ← DeerFlow内化：智能挖掘协议
...
### agui.py (NEW - 内化自 ag-ui-protocol/ag-ui)
**文件功能**：实现AG-UI协议，标准化AI智能体与前端用户界面之间的交互通信  
**技术实现**：基于事件驱动架构，使用Pydantic模型定义协议消息，通过SSE格式流式传输事件  
**关键组件**：
- 核心类型：Message(用户/助手/工具/活动消息)、Event(运行/文本/工具/活动/状态事件)
- AgUiEncoder：将事件对象编码为Server-Sent Events格式
- AgUiCallbackHandler：继承自BaseCallbackHandler，拦截AgenticX内部事件并转换为AG-UI标准事件流
- ActivitySnapshotEvent：专用于展示挖掘步骤等UI专属进度，不污染LLM上下文
- StateDeltaEvent：基于JSON Patch(RFC 6902)的状态增量同步机制
**业务逻辑**：
- 将AgenticX的TaskStartEvent/TaskEndEvent/LLMResponseEvent等内部事件映射为AG-UI的RunStarted/TextMessageChunk/RunFinished等标准事件
- 支持挖掘任务的中间步骤通过ActivityMessage实时推送到前端，避免将大量中间状态塞入对话历史
- 支持复杂状态对象(如MiningPlan)的增量更新，减少网络传输开销
**依赖关系**：依赖observability.callbacks.BaseCallbackHandler和core.event模块，为AgentExecutor提供前端交互能力
...
### mining_protocol.py (增强版，内化自 DeerFlow + AgentScope)
**文件功能**：定义智能挖掘/探索任务的结构化数据模型，支持与 AgentScope 的深度兼容
**技术实现**：Pydantic 数据模型，支持状态机转换与结构化成果提取
**关键组件**：
- `MiningStepStatus` 枚举：兼容 AgentScope 的 SubTask 状态（todo/in_progress/done/abandoned），提供 `from_agentscope` 与 `to_agentscope` 转换逻辑
- `MiningStep` 模型（增强）：
  - 新增 `outcome`: 记录步骤执行后的实际成果数据
  - 新增 `expected_outcome`: 定义步骤执行前的预期目标
  - 新增 `created_at` / `finished_at`: 精确追踪步骤生命周期时间线
  - 新增 `to_oneline_markdown()`: 生成单行摘要，优化计划视图展示
- `MiningPlan` 模型（增强）：
  - 新增 `current_subtask_idx`: 实时追踪当前正在执行的步骤索引，适配 PlanNotebook 的顺序执行逻辑
**业务逻辑**：实现从"静态计划"向"动态状态机"的演进。增强后的协议不仅定义了任务该如何做，还完整记录了任务做的结果（Outcome），为 Discovery Loop 提供了结构化的输入源
**依赖关系**：被 `agents/mining_planner_agent.py` 与 `core/plan_notebook.py` 使用
...
6. **智能挖掘协议**（DeerFlow内化）：结构化探索计划，自动约束验证，防止LLM幻觉
7. **Agent-User交互协议**（ag-ui内化）：事件驱动的前端交互标准，支持流式文本/工具调用/活动进度/状态增量同步，解决挖掘任务"黑盒"问题
