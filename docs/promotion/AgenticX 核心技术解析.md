...
记忆是智能体能否持续工作的关键。AgenticX 当前接线聚焦在可检查的运行时表面：工作区 Markdown、会话草稿、短期记忆、MCP 记忆和压缩前刷写。每类数据都有明确的落盘位置和注入点，便于追踪与替换。

## 五、工具系统与通信协议

工具系统提供统一接口，一个函数加上装饰器就能变成智能体可调用的工具。在此之上，它集成了 MCP Hub 做多服务聚合，支持远程工具、OpenAPI 工具集，并能把技能打包成可复用的技能包。Studio 的 shell 命令默认受操作系统级工作区写入边界约束。

为了让大体积的工具结果不撑爆上下文，框架引入了统一卸载机制：通过 Offloader 协议和文件后端，把大块返回值或压缩后的上下文移出实时历史，只在原地留一个引用占位符，需要时再按需取回。
...
其中比较独特的是文档脑架构。框架把文档知识库隔离成可按分身或会话挂载的脑，并支持跨脑聚合检索。

![多脑知识架构](./images/multi-brain.png)
文档脑各自维护资料和索引边界，让同一个智能体平台能服务不同知识范围的用户，而不会互相污染。代码探索则复用透明的文件读取、grep/glob 与 LSP 工具，不维护另一套语义代码索引。
...
安全在 AgenticX 里是和功能并列的一等公民。安全层包含泄露检测、输入清洗、注入攻击识别、策略引擎和输入校验，策略引擎支持基于规则、严重级别和动作的组合。Studio shell 默认在操作系统级工作区写入边界内执行；需要越界写入时必须显式申请完整访问并经过受保护确认。会话层面则做了数据库持久化、写锁和按租户的多租户隔离。
